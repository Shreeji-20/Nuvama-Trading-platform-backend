...
from nuvama.strategy_helpers import ( 
...
class StratergyDirectIOCBoxDynamicStrikes:
        ...
        try:
                ...
                if leg1_price > 0 and leg2_price > 0:  # Valid prices
                    ...
                    if valid_samples <= 3:
                        self.logger.debug(
                            ...
                            f"Fresh data fetched at {current_time:.2f}"
                        )
                else:
            ...
            if len(leg1_prices) < 10:
                observation_result = {
                    ...
                    "valid_samples": len(leg1_prices),
                    "case_decision": "CASE_A",
                    "reason": "insufficient_data",
                    "status": "defaulted_to_stable",
            ...
            if leg1_trend == "STABLE" and leg2_trend == "STABLE":
                case_decision = "CASE_A"
                decision_reason = "both_legs_stable"
                execution_order = {"primary_leg": leg1_key, "secondary_leg": leg2_key}
                ...
                observation_result = {
                    ...
                    "trends": {
                        leg1_key: {
                            "trend": leg1_trend,
                            "change": round(leg1_change, 2),
                            "volatility": round(leg1_volatility, 2),
                            "direction": leg1_direction,
                            "price_range": {
                                "start": round(leg1_prices[0], 2),
                                "end": round(leg1_prices[-1], 2),
                                "min": round(min(leg1_prices), 2),
                                "max": round(max(leg1_prices), 2)
                            }
                        },
                        leg2_key: {
                            "trend": leg2_trend,
                            "change": round(leg2_change, 2),
                            "volatility": round(leg2_volatility, 2),
                            "direction": leg2_direction,
                            "price_range": {
                                "start": round(leg2_prices[0], 2),
                                "end": round(leg2_prices[-1], 2),
                                "min": round(min(leg2_prices), 2),
                                "max": round(max(leg2_prices), 2)
                            }
                        }
                    },
                    ...
                    "isExit": isExit,
                    "price_data": {
                        leg1_key: [round(p, 2) for p in leg1_prices],
            ...
            if not isExit and self.params.get("action") == "BUY":
                if abs(leg1_change) > abs(leg2_change):
                    first_leg, second_leg = leg1_key, leg2_key
            ...
            observation_result = {
                ...
                "execution_order": {
                    ...
                    "primary_change": round(primary_change, 2),
                    "secondary_change": round(secondary_change, 2)
                },
                "symbol": self.params.get('symbol', 'NIFTY'),
                "atm_strike": self.current_atm_strike,
                "action": self.params.get("action", "BUY"),
    ...
    def _execute_both_pairs(self, uid, prices, isExit=False):
        ...
        try:
            ...
            quantities_reached, leg_key, current_qty, desired_qty = self._check_desired_quantity_reached(uid,isExit)
        ...
        try:
            while True:
                ...
                self.execution_tracker.add_milestone("Starting 10-second BUY pair observation for case decision", {
                    ...
                    "observation_duration": self.params.get("case_decision_observation_time",60)
                })
                ...
                if success:
                    ...
                    self.logger.warning(f"Dual strategy execution failed for user {uid} with {case_type}, retrying...")
                    self.execution_tracker.add_milestone(f"User {uid} dual strategy execution failed, retrying", {
                        "user": uid,
                        "strategy": case_type,
                        "status": "failed"
                    })
    ...
    def _execute_case_a_sell_first(self, uid, prices, buy_leg_keys, sell_leg_keys,observation,isExit=False):
        ...
        try:
            ...
            self.logger.info(f"Global observation SELL pair result Determined sequence: {first_sell_leg}, {second_sell_leg}, Reason: {execution_reason}")
            if not first_sell_leg or not second_sell_leg:
                return False
            elif self.params.get("action","BUY").upper() == "SELL" and (first_sell_leg != observation['first_leg'] or second_sell_leg != observation['second_leg']):
                self.logger.warning("SELL leg sequence from global observation does not match initial observation, Aborting Execution", str(sell_leg_keys))
                return False
    ...
    def _execute_case_b_buy_first(self, uid, prices, buy_leg_keys, sell_leg_keys, buy_observation,isExit=False):
        ...
        try:
            ...
            print(f"INFO: BUY legs execution order determined: {first_buy_leg}, {second_buy_leg} | {execution_reason}")
            if not first_buy_leg or not second_buy_leg:
                return False
            elif self.params.get("action","BUY").upper() == "BUY" and (first_buy_leg != buy_observation['first_leg'] or second_buy_leg != buy_observation['second_leg']):
                print(f"WARNING: BUY leg sequence from global observation does not match initial observation, Aborting Execution: {first_buy_leg}, {second_buy_leg}")
                return False
    ...
    def _monitor_sell_profit_and_execute_buy(self, uid, sell_executed_spread, remaining_spread, buy_leg_keys, profit_threshold,isExit=False):
        ...
        try:
            ...
            while True:
                ...
                if (abs(current_buy_spread) <= abs(remaining_spread) and not isExit) or (abs(current_buy_spread) >= abs(remaining_spread) and isExit):
                    ...
                    if not first_buy_leg or not second_buy_leg:
                        print(f"ERROR: Invalid BUY leg sequence from observation")
                        continue
    ...
    def _monitor_buy_profit_and_execute_sell(self, uid, buy_executed_spread, remaining_spread, sell_leg_keys, profit_threshold,isExit=False):
        ...
        try:
            ...
            while True:
                ...
                # Check if SELL spread is favorable
                if (abs(current_sell_spread) >= abs(remaining_spread) and not isExit) or (abs(current_sell_spread) <= abs(remaining_spread) and isExit):
                    ...
                    if not first_sell_leg or not second_sell_leg:
                        print(f"ERROR: Invalid SELL leg sequence from observation")
                        continue
